<!--
  ...
  ── 三條硬規則（都不是喜好問題，各有出處）──
  1. **圓角是 --radius-md（8px），不是藥丸**：全站圓角收在 4–12px，--radius-full 只給頭像與 chip。
     一顆 999px 的按鈕擺在 8px 的輸入框與 10px 的卡片旁邊，會是整頁最不像同一套設計的東西。
  2. **不用 box-shadow**：陰影只給浮動層，卡片與按鈕一律不用。層次改用顏色深淺與 1px 深色邊界
     表示——底色是暖米白，深綠色塊本身就夠跳。
  3. **高度吃 --control-h（40px）**：同一排的日期輸入框吃同一個 token，兩者底部對齊後高度差才是 0；
     各自寫死數值會差幾 px，肉眼看得出來但很難查。
-->
<template>
  <button
    :type="type"
