...
行情端點免登入，因此套上依來源位址計算的速率限制，預設每分鐘 60 次；
超過時回 `429` 並帶 `Retry-After`。要調整就覆寫這個區段：
